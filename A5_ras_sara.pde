...
//you are on branch new_data_over_time
...
void setup() {
  size(1200, 405);
  data = new FloatTable("earthquake_magnitudes.tsv");
...
void drawAxisLabels() {
  ...
  text("Magnitude", labelX, (plotY1+plotY2)/2);
  textAlign(CENTER);
  ...
  for (int row = 0; row < rowCount; row++) {
    if (data.isValid(row, col)) {
      ...
      if (dist(mouseX, mouseY, x, y) < 3) {
        ...
        textSize(10);
        textAlign(CENTER);
        text(nf(value, 0, 2) + " (" + years[row] + ")", x, 100);
        textAlign(LEFT);
      }
    }
  }
